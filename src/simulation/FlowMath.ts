...
const EPSILON = 0.05;
const AXIS_A = new THREE.Vector3(12.9898, 78.233, 37.719);
const AXIS_B = new THREE.Vector3(39.3468, 11.135, 83.155);
const AXIS_C = new THREE.Vector3(73.156, 52.235, 9.151);

function hashWave(x: number, y: number, z: number, axis: THREE.Vector3, time: number): number {
...
const targetNoise = new THREE.Vector3();

/** Add procedural divergence-free curl noise (vortex turbulence) to velocity */
export function addTurbulence(velocity: THREE.Vector3, position: THREE.Vector3, time: number, strength: number): void {
  const s = Math.max(1e-4, 0.5); // noise spatial scale
  ...
  targetNoise.set(dC_dY - dB_dZ, dA_dZ - dC_dX, dB_dX - dA_dY);
  const length = targetNoise.length();
  
  if (length > 1e-5) {
    targetNoise.multiplyScalar(1 / length);
  }
  
  velocity.addScaledVector(targetNoise, strength);
}

/** Compute a damped velocity (drag) */
export function applyDrag(velocity: THREE.Vector3, drag: number, dt: number): void {
